...
class PackDetailsController extends BaseController {
	...
	*/
	...
	public function rateUser($id){
		...
		$userRate = DB::select("select * from rates where id_user_rated = ? AND from_user = ?", array($pack -> user_id, $user -> id));
		...
		if(sizeof($myPetitions->toArray()) > 0){
			...
			$userRated = User::find($pack -> user_id);
			$userRates = DB::select("select * from rates where id_user_rated = ?", array($pack -> user_id));
			$averageRate = 0;
			for($i = 0; $i < count($userRates) ; $i++) { 
				$averageRate+= $userRates[$i]-> rate;
			}
			//$averageRate = $averageRate / count($userRates);
